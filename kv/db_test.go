...
func createTestClient(t *testing.T, addr string) *client.KV {
	...
	return client.NewKV(context, client.NewHTTPSender(addr, httpClient))
}
...
func TestKVDBContentType(t *testing.T) {
	...
	for i, test := range testCases {
		...
		httpReq, err := http.NewRequest("POST", "https://"+s.ServingAddr()+kv.DBPrefix+"Put", bytes.NewReader(body))
		if err != nil {
			t.Fatalf("%d: %s", i, err)
		}
